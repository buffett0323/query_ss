...
os.chdir('amt/src')  # equivalent to %cd in Jupyter

# === Step 2: Add the current directory to sys.path ===
sys.path.append('.')  # Ensures Python can import from this directory
...
add_safe_globals([TaskManager])


def load_model_checkpoint(args=None, device=None):
...
def transcribe_get_notes(model, audio_info):
    # Load audio
    if audio_info['filepath'].endswith('.mp3') or audio_info['filepath'].endswith('.wav'):
        audio, sr = torchaudio.load(audio_info['filepath'])
        audio = torch.mean(audio, dim=0).unsqueeze(0)  # convert to mono
        audio = torchaudio.functional.resample(audio, sr, model.audio_cfg['sample_rate'])
    elif audio_info['filepath'].endswith('.npy'):
        audio = torch.from_numpy(np.load(audio_info['filepath']))
    else:
        raise ValueError(f"Unsupported audio format: {audio_info['filepath']}")
    ...
    audio_segments = torch.from_numpy(audio_segments.astype('float32')).to(model.device).unsqueeze(1)

    # Inference
    pred_token_arr, _ = model.inference_file(bsz=8, audio_segments=audio_segments)

    # Post-process: convert tokens to note events
    start_secs_file = [
    ...
    # Write MIDI
    write_model_output_as_midi(pred_notes, 'output/',
                              audio_info['track_name'], model.midi_output_inverse_vocab)

    midifile =  os.path.join('output/model_output/', audio_info['track_name']  + '.mid')
    assert os.path.exists(midifile)
    return pred_notes, midifile
