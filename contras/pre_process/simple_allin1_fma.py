...
if __name__ == "__main__":
    ...
    folder_names = [os.path.join(input_path, f"{mp3}.mp3") for mp3 in mp3_files if mp3 not in json_folder]
    print("Filter:", len(folder_names))
    
    new_folder_names = [folder_names[i:i + chunk_size] for i in range(0, len(folder_names), chunk_size)]
    print("Process count:", len(new_folder_names))
    
    # Process
    for file_path in tqdm(new_folder_names):
