...
def save_samples(args, accelerator, tracker_step, wrapper):
    ...
    for conv_type in wrapper.convert_type:
        ...
        for i, sample_idx in enumerate(args.val_idx):
            ...
            recon_path = os.path.join(args.save_path, 'sample_audio', f'iter_{tracker_step}', f'mask_conv_{conv_type}', f'{sample_idx:02d}_recon.wav')
            recon_gt_path = os.path.join(args.save_path, 'sample_audio', f'iter_{tracker_step}', f'mask_conv_{conv_type}', f'{sample_idx:02d}_gt.wav')
            orig_path = os.path.join(args.save_path, 'sample_audio', f'iter_{tracker_step}', f'mask_conv_{conv_type}', f'{sample_idx:02d}_orig.wav')
            ref_path = os.path.join(args.save_path, 'sample_audio', f'iter_{tracker_step}', f'mask_conv_{conv_type}', f'{sample_idx:02d}_ref.wav')
...
def validate(args, accelerator, val_paired_loader, wrapper):
    ...
    for i, paired_batch in enumerate(val_paired_loader):
        for conv_type in wrapper.convert_type:
            ...
            output.update(validate_step_mask(args, accelerator, paired_batch, wrapper, conv_type))
...
# @timer
@torch.no_grad()
def validate_step_mask(args, accelerator, batch, wrapper, conv_type):
    wrapper.generator.eval()
    ...
    output[f"gen/mel-loss_{conv_type}_mask"] = wrapper.mel_loss(recons, target_audio) # Add for convert type
