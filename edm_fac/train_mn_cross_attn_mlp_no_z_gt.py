...
class Wrapper:
    def __init__(
        ...
        self.generator = dac.model.MyDAC(
            ...
            use_z_mlp_loss=args.use_z_mlp_loss,
        ...
        self.l1_loss = L1Loss().to(accelerator.device) # Designed for AudioSignal
        ...
        self.z_mlp_loss = nn.BCEWithLogitsLoss().to(accelerator.device)
        ...
        if args.use_z_mlp_loss:
            self.params["pred/z_mlp_loss"] = 10.0
...
def validate(args, accelerator, val_paired_loader, wrapper):
    ...
    for i, paired_batch in enumerate(val_paired_loader):
        for conv_type in wrapper.convert_type:
            output = validate_step(args, accelerator, paired_batch, wrapper, conv_type)
