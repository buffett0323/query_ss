...
# Buffett Added
class SeqPerturbReverse(BaseTransform):
    """Applies sequence perturbation by segmenting the audio, reversing some segments,
    and shuffling them. This transform divides the audio into a front portion (kept intact)
    ...
    def __init__(
        ...
        reverse_prob: tuple = ("uniform", 0.5, 1),
        name: str = None,
        prob: float = 1.0,
    ):
        super().__init__(name=name, prob=prob)
    ...
    def _instantiate(self, state: RandomState):
        return{
            "num_segments": self.num_segments, #util.sample_from_dist(self.num_segments, state),
            "fixed_second": self.fixed_second, #util.sample_from_dist(self.fixed_second, state),
            "reverse_prob": util.sample_from_dist(self.reverse_prob, state),
        }
